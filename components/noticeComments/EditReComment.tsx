...
const EDIT_NOTICE_RECOMMENT_MUTATION = gql`
  mutation editNoticeReComment($id: String!, $payload: String!) {
    editNoticeReComment(id: $id, payload: $payload) {
      ok
      error
    }
  }
...
export default function EditReComment({
  ...
  const editNoticeReCommentUpdate = (cache: any, result: any) => {
    const { payload } = getValues();
    setValue("payload", "");
    const {
      data: {
        editNoticeReComment: { ok, id },
      },
    } = result;
  ...
  const [editNoticeReCommentMutation, { loading: editCommentloading }] =
    useMutation(EDIT_NOTICE_RECOMMENT_MUTATION, {
      update: editNoticeReCommentUpdate,
    });
