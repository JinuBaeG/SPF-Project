...
export default function CommentComp({
  ...
  const deleteToggle = (cache: any, result: any) => {
    ...
    if (ok) {
      const boardCommentId = `NoticeComment:${id}`;
      const boardId = `Notice:${notice.id}`;
      // 삭제된 댓글 캐시에서 삭제
  ...
  return (
    <CommentContainer>
      ...
      <ActionWrapper>
        <Actions
          onPress={() => {
            navigation.navigate("NoticeReComments", {
              id,
              editComment: false,
            });
          }}
