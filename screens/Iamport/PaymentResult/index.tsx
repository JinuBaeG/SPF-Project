...
const UPDATE_CONTEST_JOIN_STATUS = gql`
...
function PaymentResult({ route, navigation }: Props) {
  ...
  const [updateContestJoinStatusMutation] = useMutation(
    UPDATE_CONTEST_JOIN_STATUS
  );

  useEffect(() => {
    if (isSuccess) {
      updateContestJoinStatusMutation({
        variables: {
          contestPaymentId: route.params?.merchant_uid,
          status: "결제완료",
        },
      });
    }
